...
if uploaded_file:
    ...
    if sheet_name:
        ...
        # Select keyword and URL columns
        ...
        # Ranking columns (all except keyword and URL)
        ranking_cols = [col for col in df.columns if col not in [keyword_col, url_col]]
        ...
        if start_date == end_date:
            ...
            df_compare = filtered_df[[keyword_col, url_col, start_date, end_date]].copy()
            df_compare.columns = ['Keyword', 'URL', 'Start Rank', 'End Rank']
            ...
            def rank_status(row):
                if pd.isna(row['Start Rank']) and pd.notna(row['End Rank']):
                    return "Dropped"   # New ranking appeared at end date (was missing at start)
                if pd.notna(row['Start Rank']) and pd.isna(row['End Rank']):
                    return "Improved"  # Was ranked at start but missing at end
            ...
            with st.expander(f"📈 Keywords Improved (missing at end) ({len(improved)})"):
                st.write(improved[['Keyword', 'Start Rank', 'End Rank', 'Change']].reset_index(drop=True))

            with st.expander(f"📉 Keywords Dropped (newly ranked) ({len(dropped)})"):
                st.write(dropped[['Keyword', 'Start Rank', 'End Rank', 'Change']].reset_index(drop=True))

            with st.expander(f"📊 Keywords Rank Up ({len(up)})"):
                st.write(up[['Keyword', 'Start Rank', 'End Rank', 'Change']].reset_index(drop=True))

            with st.expander(f"📉 Keywords Rank Down ({len(down)})"):
                st.write(down[['Keyword', 'Start Rank', 'End Rank', 'Change']].reset_index(drop=True))
            ...
            # Scatter plot Start Rank vs End Rank (only where both exist)
